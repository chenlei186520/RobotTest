...
def parse_csv_file(file_path):
    ...
    if len(rows) > 0:
        headers = rows[0]
        print(f"\n表头: {headers}")
        print(f"  列1: {headers[0] if len(headers) > 0 else ''}")
        print(f"  列2: {headers[1] if len(headers) > 1 else ''} (X060/X080车型)")
        print(f"  列3: {headers[2] if len(headers) > 2 else ''}")
        print(f"  列4: {headers[3] if len(headers) > 3 else ''} (X150/X100车型)")
        print(f"  列5: {headers[4] if len(headers) > 4 else ''}")
    ...
    # 按钮名称到ID的映射规则
    def name_to_id(name):
        name_clean = name.replace('（需要按一段时间）', '').replace('键', '')
